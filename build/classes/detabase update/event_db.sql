...
-- ============================================================
-- Billing_and_Cost Form - Query Reference
-- ============================================================

-- [SEARCH] Main billing query (btnSearch):
-- Joins events -> clients -> package -> billing to get all info.
...
-- [EXTRA COST] Sum of extra/custom resources (package_id IS NULL = added outside base package):
-- SELECT SUM(total_cost) AS extra_total
-- FROM event_resources
-- WHERE event_id = ? AND package_id IS NULL;

-- [UPDATE STATUS] (btnUpdateStatus):
-- UPDATE billing SET payment_status = ? WHERE event_id = ?;

-- ============================================================
-- Ensure billing record exists for all events (optional trigger)
-- ============================================================
-- INSERT IGNORE INTO billing (bill_id, event_id, total_amount, advance_payment, balance_due, payment_status)
-- SELECT CONCAT('B-', LPAD((SELECT COUNT(*)+1 FROM billing), 4, '0')),
--        event_id, 0, 0, 0, 'Pending'
-- FROM events WHERE event_id NOT IN (SELECT event_id FROM billing);
